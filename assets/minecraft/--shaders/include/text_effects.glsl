...
void apply_waving_movement(float speed, float frequency) {
    textData.uv.y += sin(textData.characterPosition.x * 0.1 * frequency - GameTime * 7500.0 * speed) / 512.0;
    textData.shouldScale = true;
}
...
void apply_shaking_movement() {
    float noiseX = noise(textData.characterPosition.x - textData.characterPosition.y * rand(vec2(GameTime*3.14, GameTime/-2))) - 0.1;
    float noiseY = noise(textData.characterPosition.x - textData.characterPosition.y * rand(vec2(GameTime/-2, GameTime*3.14))) - 0.1;
    textData.shouldScale = true;

    textData.uv += vec2(noiseX, noiseY) / 1548.0;
}
...
void apply_outline(vec3 color) {
    ...
    vec2 texelSize = 1.0 / vec2(3192.0);
